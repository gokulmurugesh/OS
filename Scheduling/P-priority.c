...
struct process{
    int pid,arrival,burst,wt,tat,priority,burst_remaining,is_completed;
};
...
int main(){
    ...
    while(completed != n){
        ...
        for(int i = 0; i<n; i++){
            if(p[i].arrival <= current_time && p[i].is_completed == 0){
                if(p[i].priority < min ||(p[i].priority == min && p[i].arrival < p[x].arrival)){
                    min = p[i].priority;
                    x = i;
                }
            }
    ...
    for(int i=0;i<n;i++){
        printf("%d\t%d\t%d\t%d\t%d\t%d\n",p[i].pid,p[i].arrival,p[i].burst,p[i].wt,p[i].tat,p[i].priority);
    }
